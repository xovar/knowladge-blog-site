...
export default function Blog({blog, setReadingTime, setBookmark}) {
  return (
    <>
      <div className='w-[100%] border-b-[1px] mb-[40px]' style={{borderBottom: '2px solid rgba(17, 17, 17, 0.15)'}}>
        ...
        <div className='flex items-center justify-between'>
          ...
          <div className='flex items-center'>
            ...
            <svg className='cursor-pointer' onClick={() => setBookmark(prevState => {
    // Check if the blog.id is already in the prevState
    const exists = prevState.some(state => state.id === blog.id);

    // If it doesn't exist, add the new blog, otherwise return the previous state unchanged
    if (!exists) {
